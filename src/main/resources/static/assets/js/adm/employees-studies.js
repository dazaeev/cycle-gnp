		...
		for (var i = 0; i < data.length; i++){
			...
			var actions =
				...
				'<i class="fa fa-pencil btn btn-warning" title="Editar" onclick="editEmployee(\'' 	+ rowEmployee.id 										+ '\',' 
																									...
																									+ '\'' + rowEmployee.universityCollege 				   	+ '\',' 
																									+ '\'' + rowEmployee.educationalLevel 					+ '\',' 
																									+ '\'' + rowEmployee.titleStatus  						+ '\',' 
																									+ '\'' + rowEmployee.certificationsAviable 				+ '\',' 
																									+ '\'' + rowEmployee.expiredCertificates 				+ '\',' 
																									+ '\'' + rowEmployee.others 							+ '\'' 
			...
			tr += '<td>'+ rowEmployee.universityCollege             +'</td>';
			tr += '<td>'+ rowEmployee.educationalLevel              +'</td>';
			tr += '<td>'+ rowEmployee.titleStatus                   +'</td>';
			tr += '<td>'+ rowEmployee.certificationsAviable         +'</td>';
			tr += '<td>'+ rowEmployee.expiredCertificates           +'</td>';
			tr += '<td>'+ rowEmployee.others                        +'</td>';
...
function editEmployee(id, name, universityCollege, educationalLevel, titleStatus, certificationsAviable, expiredCertificates, others) {
	$('#add-mod-employee').text('Información de ' + name);
	//
	ID_EMPLOYEE_STUDIES = id;
	cleanForm();
	$('#name').val(name);
	$('#universityCollege').val(universityCollege);
	$('#educationalLevel').val(educationalLevel);
	$('#titleStatus').val(titleStatus);
	$('#certificationsAviable').val(certificationsAviable);
	$('#expiredCertificates').val(expiredCertificates);
	$('#others').val(others);
	//
	showAddEmployee();
}
...
function confirmDeleteEmployee(name, id, active) {
    $.jAlert({
        'type': 'confirm',
        'title': 'Confirmación',
        'confirmQuestion': '¿ Seguro de eliminar información básica a </br>' + name + ' ?',
        'confirmBtnText': 'Si',
...
function saveEmployee() {
	...
	employeestudies.universityCollege			   = '' + $('#universityCollege').val();
	employeestudies.educationalLevel               = '' + $('#educationalLevel').val();
	employeestudies.titleStatus                    = '' + $('#titleStatus').val();
	employeestudies.certificationsAviable          = '' + $('#certificationsAviable').val();
	employeestudies.expiredCertificates      	   = '' + $('#expiredCertificates').val();
	employeestudies.others                         = '' + $('#others').val();
